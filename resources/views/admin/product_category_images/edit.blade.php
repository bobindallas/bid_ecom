...
@section('content_header')
	{{ Breadcrumbs::render('product_category_images.edit', $product_category) }}
@stop

@section('content')
<div class="container">
			<div id="plist">
			<form action="{{ route('product_category_images.update', [$product_category->id, $image->id]) }}" name="F1" id="F1" enctype="multipart/form-data" method="POST">
				@csrf
				@method('PUT')
				<div class="card">
				<div class="panel panel-primary">
					<div class="card-header">Update Product Category Image</div>
					<div class="card-body">
			...
			<input type="hidden" name="product_category_image_id" id="product_category_image_id" value="{{ $image->id }}">
			<input type="hidden" name="product_category_image_delete" id="product_category_image_delete" v-model="fdata.delete" value=0>
			<button type="submit" id="F1.submit" class="btn btn-primary" :disabled=fdata.disabled>Save</button>
			<button type="button" name="delete" id="delete" class="btn btn-danger" :disabled=fdata.disabled onclick="if(window.confirm('Really delete this product category image?')){ document.getElementById('product_category_image_delete').value = 1; F1.submit();}">delete</button>
			</form>
<br />
	</div>
</div>
