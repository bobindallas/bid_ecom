...
@section('content')
	<div class="container">
		...
		<div class="card">
			...
			<div class="card-body">
				<form method='POST' action="{{ route('product_options.update', $product_option->id) }}" name="F1" id="F1">
					@csrf
					@method('PUT')
					...
					<div class="form-group">
						...
						<input type="text" name="value" value="" class='form-control' v-model="fdata.value" v-on:keyup="check_form" placeholder='Product Category Value'>
					</div>
					<div class="form-group">
...
<script>
	...
	var plist = new Vue({
		...
		data: {
			...
			fdata : {
				slug            : "{{ $product_option->slug }}",
				name            : "{{ $product_option->name }}",
				display_order   : "{{ $product_option->display_order }}",
				active          : {{ $product_option->active }},
				disabled        : true,
			},
